#!/usr/bin/env bash
# Generate README + web favicon/PWA + docs-site assets from assets/brand SVGs.
# App UI keeps the dark mark; docs uses the teal inverted mark so tabs differ.
...
MARK_DOCS="$BRAND/logo-mark-docs.svg"
LOGO="$BRAND/logo.svg"
...
# Square mark variants (application / dark)
...
# Multi-resolution favicon.ico (application)
magick "$BRAND/logo-mark.png" -define icon:auto-resize=64,48,32,16 "$BRAND/favicon.ico"

# Docs mark (teal inverted) — distinct from app favicon in browser tabs
rsvg-convert -w 512 -h 512 "$MARK_DOCS" -o "$BRAND/logo-mark-docs.png"
rsvg-convert -w 32 -h 32 "$MARK_DOCS" -o "$BRAND/favicon-docs-32.png"
rsvg-convert -w 16 -h 16 "$MARK_DOCS" -o "$BRAND/favicon-docs-16.png"
magick "$BRAND/logo-mark-docs.png" -define icon:auto-resize=64,48,32,16 "$BRAND/favicon-docs.ico"

# Copy into web public (application mark)
...
# Copy into docs brand assets (docs favicon + shared logos)
cp "$BRAND/logo-mark.svg" "$DOCS_BRAND/logo-mark.svg"
cp "$BRAND/logo-mark-docs.svg" "$DOCS_BRAND/logo-mark-docs.svg"
cp "$BRAND/logo.svg" "$DOCS_BRAND/logo.svg"
cp "$BRAND/logo.png" "$DOCS_BRAND/logo.png"
cp "$BRAND/favicon-docs.ico" "$DOCS_BRAND/favicon.ico"
cp "$BRAND/favicon-docs-32.png" "$DOCS_BRAND/favicon-32.png"
cp "$MARK_DOCS" "$DOCS_BRAND/favicon.svg"
