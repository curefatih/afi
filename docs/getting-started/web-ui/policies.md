...
Open **Governance → Policies** to create custom request policies. Org owners and admins can add, edit, reorder, enable/disable, and delete policies. Creating or updating a policy publishes a new gateway snapshot so the change takes effect on the next request.
...
Related: [Web UI overview](../web-ui.md), [MCP and A2A](mcp-a2a.md).
