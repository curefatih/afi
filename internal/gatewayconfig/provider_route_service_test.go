...
func (m *memRoutes) Update(context.Context, string, string, string, string, []RouteFallback, *RetryConfig, string, int) (*Route, error) {
	return nil, nil
}
...
func TestCreateRouteRejectsCrossOrgProvider(t *testing.T) {
	...
	routes := &memRoutes{}
	_, err := CreateRoute(context.Background(), routes, providers, "route_1", "org_a", "gpt-4o", "prov_other", "gpt-4o", nil, nil, "", 0)
...
func TestCreateRouteAcceptsSameOrgProvider(t *testing.T) {
	...
	routes := &memRoutes{}
	r, err := CreateRoute(context.Background(), routes, providers, "route_1", "org_a", "gpt-4o", "prov_ok", "gpt-4o", nil, nil, "", 0)
	if err != nil {
		t.Fatal(err)
	}
